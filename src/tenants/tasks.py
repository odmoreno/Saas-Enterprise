...
def migrate_tenant_task(tenant_id: str):
    ...
    try:
        ...
        print(f"Tenant with id {tenant_id} failed {e}.")
        return
    ...
    with use_tenant_schema(schema_name=schema_name, create_if_missing=True, revert_public=True):
        ...
        for app_config in customer_app_configs:
            ...
            print(
                f"Applying migrations for app '{app_label}':"
            )
...
def migrate_tenant_schemas_task():
    ...
    with use_public_schema():
        qs = Tenant.objects.all().values_list("id", flat=True)  # Get all tenant IDs
        call_command("migrate", interactive=False)
